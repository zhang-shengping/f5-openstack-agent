...
"$blk1[string repeat "0000" [expr {8 - [string length "$blk1$blk2"]/4}]]$blk2"
    ...
    def create_iRule(self, service, vip, bigip, **kwargs):
        ...
        irule_name = self.get_irule_name(service, "insert-clientip-TCP-option")
        ...
        if not irule_exists:
            ...
            LOG.info(
                "Add insert-clientip-TCP-option iRule: {} for "
                "BIGIP: {} ".format(
        ...
        # pzhang: when create new, we do not append
        vip['rules'] = [irule_fullPath]
        # vip['rules'].append(irule_fullPath)

#     pzhang: we need to remove and update
