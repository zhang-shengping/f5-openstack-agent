...
class iRule(object):
    ...
    # action_mapping = {
    #     "REDIRECT_TO_POOL": '''when HTTP_REQUEST {if {[HTTP::%s] matches_regex {%s}} {pool %s}}''',
    #     "REDIRECT_TO_URL": '''when HTTP_REQUEST {if {[HTTP::%s] matches_regex {%s}} {HTTP::uri %s}}'''
    # }
    action_mapping = {
    ...
    type_mapping = {"HOST_NAME": "host", "PATH": "path", "FILE_TYPE": "????", 
    ...
    def __init__(self, policy):
        ...
        # self._adapt_regex_to_irule(policy, service, partition, env_prefix)
            ...
            if os_rule['provisioning_status'] != 'PENDING_DELETE' and \
               os_rule['admin_state_up']:
                if os_rule['compare_type'] == "REGEX":
                    ...
                    irule_object = self._l7rule_to_irule(partition,
                            env_prefix, rule_id)
    ...
    def _l7rule_to_irule(self, partition, env_prefix, rule_id):
        ...
        if self.action == "REDIRECT_TO_POOL":
            template = self.action_mapping[self.action]
            pool_name = self._get_pool_name(partition, env_prefix, self.action_pool)
            self.apiAnonymous = template%(self.type, self.value, pool_name)
        if self.action == "REDIRECT_TO_URL":
            template = self.action_mapping[self.action]
            self.apiAnonymous = template%(self.type, self.value, self.action_uri)
